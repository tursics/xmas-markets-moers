...
config.startDate.setFullYear( 2014, 10, 24); // month - 1
config.endDate = new Date();
config.endDate.setFullYear( 2015, 0, 4); // month - 1
...
if( document.querySelector('#tab-market-tomorrow') !== null) {
...
function sortDataOvermorrow( a, b)
{
	var timeA = getOpeningTime( a, 2);
	var timeB = getOpeningTime( b, 2);
...
function fillListDaysMarkets()
{
	setActiveTab( '#tab-market-days');
...
if( document.querySelector('#tab-market-days') !== null) {
	document.querySelector('#tab-market-days').addEventListener('click', function() {
		fillListDaysMarkets();
	});
}
...
//	txt += '<li style="float:left;padding:0 2rem 0 0;"><button>BVG</button></li>';
	...
	txt += '<p>Datum: ' + obj.date + '<br>Uhrzeit: ' + obj.hours + '</p>';
...
} else if( document.querySelector('#tab-market-days') !== null) {
	setTimeout( fillListDaysMarkets, config.timeout);
}
